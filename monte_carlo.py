...
Implements stochastic error injection using a validated LEO radiation
...
• Burst size distribution:
      ...
      5–16 bits       12 %   (high-LET heavy ions)
      17–64 bits       3 %   (very high-LET / direct hits)
...
def sample_burst_size(rng: np.random.Generator) -> int:
    """Sample one burst size from the validated LEO distribution.

    Returns an integer in [2, 64].
    """
    u = rng.random()
    if u < _BURST_CDF[0]:           # 55 %  →  2 bits
        return 2
    elif u < _BURST_CDF[1]:         # 30 %  →  3–4 bits (uniform)
        return int(rng.integers(3, 5))
    elif u < _BURST_CDF[2]:         # 12 %  →  5–16 bits (uniform)
        return int(rng.integers(5, 17))
    else:                            #  3 %  →  17–64 bits (uniform)
        return int(rng.integers(17, 65))
