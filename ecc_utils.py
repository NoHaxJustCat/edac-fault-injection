...
def corrupt_page_leo(sectors, seu_rate, scrub_interval, rng=None):
    ...
    Drop-in replacement for :func:`corrupt_page` that uses the assumed
    LEO radiation environment model (10:1 SEU-to-burst ration).
    ...
    Returns:
        list of corrupted numpy arrays (same structure as input)
    """
    from error_models import inject_errors_leo
    if rng is None:
        rng = np.random.default_rng()
    corrupted = []
    for sector in sectors:
