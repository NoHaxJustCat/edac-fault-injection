...
Usage
-----
    python ecc_simulator.py                                     # analytical mode (default)
    python ecc_simulator.py --analytical                        # explicit analytical-only mode
    python ecc_simulator.py --monte-carlo                       # run full Monte Carlo simulation
    python ecc_simulator.py --mode burst                        # with LEO burst model
    python ecc_simulator.py --monte-carlo --mc-iter 1250        # custom iterations (default: 10)
    python ecc_simulator.py --monte-carlo --mode burst          # combine flags

Flags
-----
    --mode {random, burst}
        Type of radiation error model.
        • 'random' (default): independent single-bit SEUs only
        • 'burst': LEO orbit radiation model with 10:1 single-to-burst SEU ratio

    --analytical
        Plot analytical (Poisson) UBER curves only, skip Monte Carlo simulation.
        Runs instantly; useful for quick theoretical comparisons.
        (default: True)

    --monte-carlo
        Run full Monte Carlo simulation instead of analytical mode.
        (default: False)

    --mc-iter N
        Override Monte Carlo iterations per (architecture × SEU-rate) point.
        Resolution floor = 1 / (N × NUM_SECTORS).
        Examples: N=1250 → 10⁻⁴ floor; N=12500 → 10⁻⁵ floor.
        (default: 10 from NUM_ITERS constant)
...
from fault_injection import inject_errors_leo, inject_errors_leo_conditional, _POPCOUNT8

# ===========================================================
#  OUTPUT DIRECTORY
# ===========================================================
_BASE_OUTPUT_DIR = os.path.join("results", "edac_uber")
...
# SEU rate sweep -- same x-axis as the original compare_ecc_single script
# (kept to match legacy analytical/MC overlays)
...
# LEO burst error distribution (compound Poisson model)
#
# LEO radiation environment has a 10:1 single-to-burst SEU ratio.
# Burst bit-size distribution (SLC, heavy ion):
#   • 2-bit bursts: 75% of events
#   • 3-4 bit bursts: 18% of events (uniformly distributed)
#   • 5-16 bit bursts: 5% of events (uniformly distributed)
#   • >16-bit bursts: <2% (likely SEFI, excluded from model)
#
# NOTE: Keep these parameters synchronized with error_models.py!
#       error_models.py uses these same values for error injection.
#
BURST_SINGLE_TO_BURST_RATIO = 10.0  # 10:1 ratio from LEO model (single:burst)
...
# ===========================================================
#  COMPOUND POISSON BURST ANALYTICAL MODELS
# ===========================================================
# Derived weight factors from LEO burst model defined in SIMULATION PARAMETERS.
# The single and burst event probabilities are computed as:
#   P(single) = 10 / (10 + 1) = 10/11
#   P(burst)  =  1 / (10 + 1) =  1/11

_W_SINGLE = BURST_SINGLE_TO_BURST_RATIO / (1.0 + BURST_SINGLE_TO_BURST_RATIO)
_W_BURST  = 1.0 / (1.0 + BURST_SINGLE_TO_BURST_RATIO)

# Burst bit-size PMF for sizes 2..16 (SLC, heavy ion)
# The distribution is renormalized by (1 - BURST_EXCLUDED_PERCENTAGE) to account
# for >16-bit bursts (likely SEFI) that are excluded from the model.
_RENORM_FACTOR = 1.0 - BURST_EXCLUDED_PERCENTAGE

_BURST_BITS_PMF = np.zeros(17)
_BURST_BITS_PMF[2] = BURST_BIT_DISTRIBUTION[2] / _RENORM_FACTOR
for _k in range(3, 5):
    _BURST_BITS_PMF[_k] = (BURST_BIT_DISTRIBUTION["3-4"] / 2) / _RENORM_FACTOR
for _k in range(5, 17):
    _BURST_BITS_PMF[_k] = (BURST_BIT_DISTRIBUTION["5-16"] / 12) / _RENORM_FACTOR


def _make_bit_event_pmf():
    """Event-size PMF (bits) for combined single+burst compound Poisson."""
    max_size = 16
    f = np.zeros(max_size + 1)
    f[1] = _W_SINGLE
    for k in range(2, 17):
        f[k] = _W_BURST * _BURST_BITS_PMF[k]
    return f

_BIT_EVENT_PMF = _make_bit_event_pmf()


def _make_byte_event_pmf():
    """Event-size PMF (bytes/symbols) for RS compound Poisson.

    Converts burst bit sizes to byte-symbol corruption counts accounting
    for random byte alignment within the codeword.
    """
    max_bytes = 3  # max from a 16-bit burst with alignment offset
    f = np.zeros(max_bytes + 1)

    # Singles: 1 byte
    f[1] += _W_SINGLE

    # Bursts: each burst of B bits corrupts ceil(B/8) or ceil(B/8)+1 bytes
    for B in range(2, 17):
        p_B = _BURST_BITS_PMF[B]
        if p_B == 0:
            continue
        k = ceil(B / 8)
        excess = 8 * k - B
        p_lower = (excess + 1) / 8.0
        p_upper = 1.0 - p_lower
        f[k] += _W_BURST * p_B * p_lower
        if k + 1 <= max_bytes:
            f[k + 1] += _W_BURST * p_B * p_upper

    return f

_BYTE_EVENT_PMF = _make_byte_event_pmf()


def _compound_poisson_sf(t, lam_total, event_pmf):
    """P(compound Poisson total > t) via Panjer recursion.

    The compound Poisson S = X_1 + ... + X_N where N ~ Poisson(lam_total)
    and X_i have the given event_pmf.  Panjer recursion computes the PMF
    g[n] = P(S = n) iteratively and returns the tail P(S > t).
    """
    if lam_total < 1e-30:
        return 0.0
    if lam_total > 500:
        return 1.0  # overwhelmed — P(fail) ≈ 1

    max_k = len(event_pmf) - 1
    g = np.zeros(t + 2)
    g[0] = np.exp(-lam_total)
    ...
    for n in range(1, t + 1):
        ...
        for k in range(1, upper + 1):
            if event_pmf[k] > 0:
                s += k * event_pmf[k] * g[n - k]
        g[n] = (lam_total / n) * s

    return max(0.0, 1.0 - g[:t + 1].sum())


def uber_bch_burst(seu_rate, bch_t, codeword_bits,
                   scrub=SCRUB_INTERVAL):
    """Analytical UBER for BCH under LEO burst model (compound Poisson).

    Total event rate = single rate × (1 + burst/single) where burst/single = 1/ratio.
    """
    lam_single = seu_rate * codeword_bits * scrub
    lam_total  = lam_single * (1.0 + 1.0 / BURST_SINGLE_TO_BURST_RATIO)
    return _compound_poisson_sf(bch_t, lam_total, _BIT_EVENT_PMF)


def uber_rs_burst(seu_rate, rs_t, nsym, sector_bytes,
                  scrub=SCRUB_INTERVAL):
    """Analytical UBER for RS under LEO burst model (compound Poisson).

    Uses byte-level event-size PMF to model symbol errors from contiguous
    burst bit-flips with random byte alignment.
    """
    num_chunks = max(1, ceil(sector_bytes / 255))
    chunk_bytes = sector_bytes // num_chunks
    chunk_bits = chunk_bytes * 8
    lam_single = seu_rate * chunk_bits * scrub
    lam_total  = lam_single * (1.0 + 1.0 / BURST_SINGLE_TO_BURST_RATIO)

    p_chunk_fail = _compound_poisson_sf(rs_t, lam_total, _BYTE_EVENT_PMF)
    return 1.0 - (1.0 - p_chunk_fail) ** num_chunks


def uber_ldpc_burst(seu_rate, t_eff, page_bits,
                    scrub=SCRUB_INTERVAL):
    """Analytical UBER for LDPC under LEO burst model (compound Poisson)."""
    lam_single = seu_rate * page_bits * scrub
    lam_total  = lam_single * (1.0 + 1.0 / BURST_SINGLE_TO_BURST_RATIO)
    return _compound_poisson_sf(t_eff, lam_total, _BIT_EVENT_PMF)
...
def _make_rs_arch(nsym, page_size, color, mode="random"):
    ...
    if mode == "burst":
        def analytical(r):
            return uber_rs_burst(r, rs_t, nsym, sector_bytes)
    else:
        def analytical(r):
...
def _make_bch_arch(ecc_bytes, page_size, color, mode="random"):
    ...
    if mode == "burst":
        def analytical(r):
            p_chunk_fail = uber_bch_burst(r, bch_t, chunk_bits)
...
def _make_ldpc_arch(page_size, mode="random"):
    ...
    if mode == "burst":
        def analytical(r):
            return uber_ldpc_burst(r, t_eff, page_bits)
    else:
        def analytical(r):
...
def build_archs(page_size, mode="random"):
    ...
    If *mode* is ``'burst'``, each architecture's analytical function uses
    the compound Poisson LEO burst model instead of the Poisson random model.
    """
    archs = []

    for nsym, color in zip([8, 16], _RS_COLORS):
        try:
            archs.append(_make_rs_arch(nsym, page_size, color, mode=mode))
        except ValueError:
            pass

    for ecc_bytes, color in zip([13, 22, 31], _BCH_COLORS):
        try:
            arch = _make_bch_arch(ecc_bytes, page_size, color, mode=mode)
            if arch is not None:
                archs.append(arch)
        except ValueError:
            pass

    archs.append(_make_ldpc_arch(page_size, mode=mode))
    return archs
...
def _worker(task):
    ...
    try:
        ...
        lam_page_single = seu_rate * (page_size * 8) * scrub_interval
        if mode == "burst":
            lam_page = lam_page_single * (1.0 + 1.0 / BURST_SINGLE_TO_BURST_RATIO)
        else:
            lam_page = lam_page_single
...
def _plot_uber(page_size, archs, results_map, mode, output_dir):
    ...
    show the analytical prediction for comparison. Matches legacy style.
    """
...
def _plot_analytical_only(page_size, archs, mode, output_dir):
    ...
    ax.set_title(
        f"ECC UBER vs SEU Rate ({mode_label})  --  page = {page_size} B, "
...
def run_all_analyses(page_sizes, mode):
    ...
    for page_size in page_sizes:
        archs = build_archs(page_size, mode=mode)
        sector_bytes = page_size // NUM_SECTORS
...
if __name__ == "__main__":
    ...
    sim_group.add_argument(
        ...
        help=(
            "Plot analytical (Poisson) UBER curves only; skip Monte Carlo. "
            "Runs instantly. (default mode)"))
    sim_group.add_argument(
        "--monte-carlo", action="store_true",
        help="Run full Monte Carlo simulation instead of analytical mode.")
    parser.add_argument(
    ...
    # Default to analytical mode if neither flag specified
    use_analytical = args.analytical or (not args.monte_carlo)
    ...
    if use_analytical:
        print("  [analytical mode] Skipping Monte Carlo -- plotting Poisson models.")
        out_dir = _get_output_dir(f"analytical/{args.mode}")
        for page_size in PAGE_SIZES:
            archs = build_archs(page_size, mode=args.mode)
